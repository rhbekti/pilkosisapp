...
class Voting extends CI_Controller
{
    ...
    public function voting_kandidat()
    {
        ...
        if($post == null){
            ...
            $this->session->set_flashdata('info','Maaf Anda Melakukan akses Ilegal!!');
            redirect('/Dashboard');
        }
        ...
        if ($tgl_sekarang >= $tglmulai) {
            if ($tgl_sekarang > $tglakhir) {
                $this->session->set_flashdata('aksi', 'error');
                $this->session->set_flashdata('info', 'Maaf Pemilihan Telah ditutup.');
                redirect('/Dashboard');
            } else {
                $ck_user = $this->M_vote->cek_sdah_voting($this->input->post('idpemilih'));
                if ($ck_user->num_rows() < 1) {
                    $data['judul'] = 'Menu Voting';
                    // untuk mencetak id pada navbar
                    $data['user'] = $this->M_user->get($this->input->post('idpemilih'))->row_array();
                    $this->load->view('template/header');
                    $this->load->view('template/navbar', $data);
                    $this->load->view('template/sidebar');
